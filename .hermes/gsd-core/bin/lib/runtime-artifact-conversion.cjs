 ...
 * Path mappings depend on install mode:
 *   Global: ~/.hermes/ → ~/.gemini/antigravity/, ./.hermes/ → ./.agents/
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Replace "Hermes Agent" brand references with "Cursor"
...
/**
 * Convert a Hermes Agent command to a Cursor 1.6 slash command (#785).
 *
 ...
 *
 * @param {string} content   raw Hermes Agent command markdown (may have frontmatter)
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Hermes Agent bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Hermes Agent" brand references with "Windsurf"
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Hermes Agent bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Hermes Agent" brand references with "Augment"
 ...
 * GSD's worktree isolation uses Hermes Agent's isolation="worktree" spawn
