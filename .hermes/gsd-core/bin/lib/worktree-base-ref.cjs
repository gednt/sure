...
function buildMsgDiverged(headSha, forkRef, forkSha) {
    return `⚠ Worktree base mismatch: HEAD (${shortSha(headSha)}) differs from ${forkRef} (${shortSha(forkSha)}). Running this phase sequentially on the main working tree. To keep parallel worktrees, set worktree.baseRef:"head" in .hermes/settings.local.json (or run: gsd-tools worktree set-baseref). See #683.`;
}
const MSG_UNKNOWN = `⚠ Cannot determine the worktree fork base (origin/HEAD unresolved). Running this phase sequentially on the main working tree to avoid a base mismatch. To keep parallel worktrees, set worktree.baseRef:"head" in .hermes/settings.local.json (or run: gsd-tools worktree set-baseref). See #683.`;
// ─── Exports ──────────────────────────────────────────────────────────────────
/**
...
function evaluateWorktreeBaseDegrade(deps) {
    ...
    // Hermes Agent's worktree.baseRef accepts only "fresh" (= origin/HEAD, the default) or "head".
    ...
    // semantics and must be evaluated against origin/HEAD. (Reference: Hermes Agent worktrees docs, #683.)
