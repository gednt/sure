 ...
 * Path mappings depend on install mode:
 *   Global: ~/.qwen/ → ~/.gemini/antigravity/, ./.qwen/ → ./.agents/
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Replace "Qwen Code" brand references with "Cursor"
...
/**
 * Convert a Qwen Code command to a Cursor 1.6 slash command (#785).
 *
 ...
 *
 * @param {string} content   raw Qwen Code command markdown (may have frontmatter)
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Qwen Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Qwen Code" brand references with "Windsurf"
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Qwen Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Qwen Code" brand references with "Augment"
 ...
 * GSD's worktree isolation uses Qwen Code's isolation="worktree" spawn
