...
const capabilities = {
  ...
  "profile-pipeline": {
    ...
    "description": "Developer behavioral profiling from Trae session history; scans session JSONL files, extracts and samples user messages, and generates profile artifacts (USER-PROFILE.md, dev-preferences.md, .trae/rules/ sections). Exposes eight `gsd-tools` commands: scan-sessions, extract-messages, profile-sample (pipeline phase) and write-profile, profile-questionnaire, generate-dev-preferences, generate-claude-profile, generate-claude-md (output phase). Backs the /gsd-profile-user skill and gsd-user-profiler agent.",
    "tier": "full",
    "requires": [],
    "engines": {
      "gsd": ">=1.6.0"
