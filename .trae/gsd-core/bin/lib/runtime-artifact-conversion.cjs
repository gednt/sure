...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Replace "Trae" brand references with "Cursor"
...
/**
 * Convert a Trae command to a Cursor 1.6 slash command (#785).
 *
 ...
 *
 * @param {string} content   raw Trae command markdown (may have frontmatter)
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Trae bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Trae" brand references with "Windsurf"
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Trae bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Trae" brand references with "Augment"
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Replace .trae/rules/ with runtime-appropriate instruction file
    if (instructionFile) {
 ...
 * GSD's worktree isolation uses Trae's isolation="worktree" spawn
