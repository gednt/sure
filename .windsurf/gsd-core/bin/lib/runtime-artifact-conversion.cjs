...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Replace "Windsurf" brand references with "Cursor"
...
/**
 * Convert a Windsurf command to a Cursor 1.6 slash command (#785).
 *
 ...
 *
 * @param {string} content   raw Windsurf command markdown (may have frontmatter)
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Windsurf bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Windsurf" brand references with "Windsurf"
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Windsurf bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Windsurf" brand references with "Augment"
 ...
 * GSD's worktree isolation uses Windsurf's isolation="worktree" spawn
