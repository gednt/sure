0.5. **Inter-wave worktree base re-check (wave N+1 guard — #1369):**
   ...
   **To avoid this degrade across all waves:** set `worktree.baseRef:"head"` in
   `.windsurf/settings.local.json` (or run `gsd-tools worktree set-baseref`). This tells
   Windsurf to fork from the live HEAD instead of `origin/HEAD`, so each wave's new
