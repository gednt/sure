...
- **Other runtimes:** If `Agent`/`agent` tool is genuinely unavailable (e.g. a backgrounded
  Windsurf agent per #853, or a non-Claude runtime), use sequential inline execution as
...
@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/agent-contracts.md
@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/context-budget.md
@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/gates.md
</required_reading>
...
# Only applies to Windsurf (isolation="worktree" is Claude-Code-specific).
if [ "$RUNTIME" = "claude" ] && [ "$USE_WORKTREES" != "false" ]; then
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/planner-antipatterns.md
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/workflows/execute-plan.md` **inline**
...
per-agent branch namespace `worktree-agent-*`, NOT the `.windsurf/worktrees/` path: the
...
@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/execute-phase-wave-guard.md

@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/execute-phase-context-guard.md
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/checkpoints.md
       @/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/tdd.md
       @/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/executor-examples.md'}
       </execution_context>
...
@/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/execute-phase-between-wave-reset.md
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/references/execute-mvp-tdd.md`): when `MVP_MODE=true` AND `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit), the end-of-phase TDD review escalates from advisory to **blocking under MVP+TDD** — refuse to mark the phase complete and present:
...
Read and follow `/home/reisfelipe18/Repos/sure/.windsurf/gsd-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
